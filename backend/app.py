...
def generate_explanation(candidate, disease_name):
    ...
    prompt = f"""You are a senior biomedical scientist and clinical pharmacologist advising a translational research team.

Drug: {candidate['drugName']}
Original Indication: {original_use}
New Target Disease: {disease_name}
Target Gene: {candidate['targetGene']} ({candidate['targetFullName']})
Open Targets Evidence Score: {candidate['confidenceScore']:.2f}/1.0
Clinical Stage: {candidate.get('clinicalStage', 'Investigational')}

Provide a concise but scientifically rigorous assessment covering:
1. MECHANISM: What is the drug's primary mechanism of action and why does its target ({candidate['targetGene']}) matter for {disease_name}?
2. REPURPOSING RATIONALE: Why is this a credible repurposing candidate based on the shared biological target overlap?
3. BIOMARKER STRATEGY: Suggest one measurable biomarker or surrogate endpoint to validate target engagement in a Phase II trial.
4. SAFETY NOTE: Flag one key safety consideration when using this drug in the {disease_name} patient population.

Keep the total response under 120 words. End by noting this is a computational hypothesis requiring experimental validation."""
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        ...
        for item in futures:
            if hasattr(item, "result"):
                try:
                    explanations.append(item.result(timeout=35))
                except Exception:
...
# ============================================================
# EXPANDED AI INTELLIGENCE ENDPOINTS
# ============================================================

def _ollama_query(prompt, max_tokens=200, temperature=0.3, timeout_sec=45):
    """Shared helper: send a prompt to Ollama, return text or None on failure."""
    if not is_ollama_available():
        return None
    try:
        resp = requests.post(
        ...
        print(f"Ollama query notice: {e}")
    return None


@app.route("/api/ai/sar-analysis")
def ai_sar_analysis():
    """
    AI-powered Structure-Activity Relationship (SAR) analysis.
    GET /api/ai/sar-analysis?smiles=CC(=O)OC1...&name=Aspirin&mw=180.16&logp=1.2&tpsa=63.6&hbd=1&hba=4&rotb=3&lipinski=true&pains=false
    """
    smiles = request.args.get("smiles", "").strip()
    name = request.args.get("name", "Compound").strip()
    mw = request.args.get("mw", "N/A")
    logp = request.args.get("logp", "N/A")
    tpsa = request.args.get("tpsa", "N/A")
    hbd = request.args.get("hbd", "N/A")
    hba = request.args.get("hba", "N/A")
    rotb = request.args.get("rotb", "N/A")
    lipinski = request.args.get("lipinski", "true")
    pains = request.args.get("pains", "false")

    if not smiles:
        return jsonify({"error": "Missing 'smiles' parameter"}), 400

    prompt = f"""You are a senior medicinal chemist performing a structure-activity relationship (SAR) assessment.

Compound: {name}
SMILES: {smiles}
Molecular Weight: {mw} Da | LogP: {logp} | TPSA: {tpsa} Å² | H-Bond Donors: {hbd} | H-Bond Acceptors: {hba} | Rotatable Bonds: {rotb}
Lipinski Compliant: {lipinski} | PAINS Alert: {pains}

Provide a concise SAR analysis covering:
1. KEY FUNCTIONAL GROUPS: Identify the pharmacologically important functional groups in this SMILES and their role in binding/activity.
2. ADMET INTERPRETATION: Explain why this molecule has its observed LogP, TPSA, and permeability profile based on its structural features.
3. OPTIMIZATION SUGGESTION: Propose one specific bioisosteric replacement or structural modification that could improve oral bioavailability or reduce a liability.
4. PAINS/TOXICOPHORE ASSESSMENT: If PAINS alerts are flagged, assess whether they represent true liabilities or likely assay interference.

Keep response under 150 words."""

    t0 = time.time()
    ai_text = _ollama_query(prompt, max_tokens=220, temperature=0.3)
    elapsed = round(time.time() - t0, 2)

    if not ai_text:
        pains_note = "No PAINS structural alerts detected — the molecule shows a clean toxicophore profile." if pains == "false" else "PAINS alerts detected — further selectivity screening recommended to rule out promiscuous binding."
        ai_text = (
            f"{name} (MW {mw} Da, LogP {logp}) shows a {'favorable' if float(logp or 0) < 5 else 'high'} lipophilicity profile. "
            f"The TPSA of {tpsa} Å² suggests {'good oral absorption and moderate membrane permeability' if float(tpsa or 0) < 140 else 'limited passive permeability'}. "
            f"With {hbd} H-bond donors and {hba} acceptors, the molecule {'satisfies' if lipinski == 'true' else 'violates'} Lipinski's Rule of Five. "
            f"{pains_note} "
            f"Consider evaluating prodrug strategies or bioisosteric replacements to optimize the pharmacokinetic profile."
        )

    return jsonify({
        "compound": name,
        "smiles": smiles,
        "sar_analysis": ai_text,
        "aiPowered": is_ollama_available(),
        "model": OLLAMA_MODEL if is_ollama_available() else "algorithmic-fallback",
        "latency_seconds": elapsed
    })
...
@app.route("/api/ai/pathway-crosstalk")
def ai_pathway_crosstalk():
    """
    AI-powered polypharmacological pathway crosstalk and synergy analysis.
    GET /api/ai/pathway-crosstalk?drug=Aspirin&targets=PTGS2,PTGS1&pathways=Arachidonic+acid+metabolism,Prostaglandin+synthesis&disease=Alzheimer's+disease
    """
    drug = request.args.get("drug", "").strip()
    targets_str = request.args.get("targets", "").strip()
    pathways_str = request.args.get("pathways", "").strip()
    disease = request.args.get("disease", "").strip()

    if not drug or not targets_str:
        return jsonify({"error": "Missing 'drug' and 'targets' parameters"}), 400

    targets = [t.strip() for t in targets_str.split(",") if t.strip()]
    pathways = [p.strip() for p in pathways_str.split(",") if p.strip()] if pathways_str else ["(not specified)"]

    prompt = f"""You are a systems pharmacologist analyzing polypharmacological network effects.

Drug: {drug}
Off-Target Proteins: {', '.join(targets)}
Affected Pathways: {', '.join(pathways)}
Disease Context: {disease if disease else 'General'}

Provide a concise polypharmacological assessment covering:
1. NETWORK EFFECT: How does simultaneously modulating these targets ({', '.join(targets)}) create synergistic or antagonistic pharmacological effects?
2. PATHWAY CROSSTALK: Explain how the affected pathways interact — do they converge on shared downstream effectors (e.g., NF-κB, mTOR, MAPK cascades)?
3. THERAPEUTIC SYNERGY: Does multi-target engagement offer therapeutic advantage for {disease if disease else 'complex diseases'}, or does it raise polypharmacological toxicity concerns?
4. NOVEL INDICATION: Based on this off-target profile, suggest one non-obvious disease indication that could benefit from this drug's multi-target activity.

Keep response under 150 words."""

    t0 = time.time()
    ai_text = _ollama_query(prompt, max_tokens=220, temperature=0.35)
    elapsed = round(time.time() - t0, 2)

    if not ai_text:
        ai_text = (
            f"{drug} engages {len(targets)} protein targets ({', '.join(targets)}), "
            f"spanning {len(pathways)} biological pathways ({', '.join(pathways[:3])}). "
            f"This multi-target profile suggests polypharmacological modulation of interconnected signaling cascades. "
            f"Concurrent modulation of these targets may produce synergistic downstream effects relevant to "
            f"{disease if disease else 'complex multifactorial diseases'}. "
            f"Further experimental validation through combinatorial bioassay panels is recommended to quantify "
            f"therapeutic synergy versus off-target toxicity risk."
        )
...
@app.route("/api/ai/compare-synthesis", methods=["POST"])
def ai_compare_synthesis():
    """
    AI-powered head-to-head comparison synthesis for up to 4 drug candidates.
    POST /api/ai/compare-synthesis
    Body: { "disease": "Alzheimer's", "candidates": [{"name": "...", "gene": "...", "score": 0.9, ...}, ...] }
    """
    ...
    if not candidates or len(candidates) < 2:
        return jsonify({"error": "Provide at least 2 candidates for comparison"}), 400

    candidate_lines = []
    for i, c in enumerate(candidates[:4], 1):
        candidate_lines.append(
            f"  {i}. {c.get('name','?')} — Target: {c.get('gene','?')} | "
            f"Score: {c.get('score','?')}/1.0 | Phase: {c.get('phase','?')} | "
            f"Known for: {c.get('origUse','?')}"
        )

    prompt = f"""You are a pharmaceutical R&D advisor conducting a head-to-head candidate evaluation.

Disease: {disease if disease else 'Target Condition'}
Candidates Under Review:
{chr(10).join(candidate_lines)}

Provide a decisive comparative assessment covering:
1. LEAD CANDIDATE: Which compound is the strongest repurposing lead and why? Consider evidence score, clinical stage, and target relevance.
2. BACKUP CANDIDATE: Which is the best secondary option if the lead fails in validation?
3. DIFFERENTIATION: What is the key pharmacological differentiator between these candidates (target specificity, safety profile, regulatory pathway)?
4. RISK FLAG: Identify the highest-risk candidate and explain why it carries more uncertainty.
5. RECOMMENDATION: State your final recommendation in one sentence.

Keep response under 180 words."""

    t0 = time.time()
    ai_text = _ollama_query(prompt, max_tokens=250, temperature=0.3)
    elapsed = round(time.time() - t0, 2)

    if not ai_text:
        sorted_cands = sorted(candidates[:4], key=lambda c: float(c.get("score", 0)), reverse=True)
        lead = sorted_cands[0]
        backup = sorted_cands[1] if len(sorted_cands) > 1 else None
        ai_text = (
            f"Based on evidence scores, {lead.get('name', 'Candidate 1')} emerges as the primary lead "
            f"with a confidence score of {lead.get('score', 'N/A')}/1.0 targeting {lead.get('gene', 'N/A')}. "
        )
        if backup:
            ai_text += (
                f"{backup.get('name', 'Candidate 2')} serves as the secondary backup "
                f"(score: {backup.get('score', 'N/A')}/1.0). "
            )
        ai_text += (
            f"All candidates require independent experimental validation including target engagement "
            f"bioassays and safety profiling before clinical advancement for {disease if disease else 'the target indication'}."
        )
